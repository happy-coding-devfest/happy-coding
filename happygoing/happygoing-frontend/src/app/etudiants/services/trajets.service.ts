...
Ambohimahitsy 08:25_AM`;
    ...
    private _trajets$ = new BehaviorSubject<string[]>([]);
    get trajets$(): Observable<string[]> {
    ...
    findPath(data: CreateTrajetModel): void {
        ...
        this.trajets$.pipe(
            map(response => {
                response.push(this.processData(input, data.depart, data.arrivee, this.formatTimeTo12H(data.heure)));
                return response;
            })
        ).subscribe();
    }
